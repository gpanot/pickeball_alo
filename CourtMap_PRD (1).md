...
- Show success state: checkmark animation + "Request sent!" + booking reference (orderId, e.g. "CM-4A7F" displayed large) + "The venue will confirm your booking shortly"
...
**Booking card:**
- Booking reference (orderId, e.g. "CM-4A7F", displayed prominently at top left)
...
**Booking status polling:** When the My Bookings screen is open, automatically refetch bookings from the API every 30 seconds. This ensures the player sees status changes (e.g. pending to booked) made by the venue admin without needing to manually refresh. Show a subtle "Updated just now" timestamp at the top of the list. No push notifications for MVP.
...
interface Booking {
  id: string;
  orderId: string; // short human-readable reference, auto-generated (e.g. "CM-4A7F")
  venueId: string;
...
- Venue owners approve/reject "pending" bookings (separate admin interface, see Admin PRD)
...
When a player submits a booking, the POST `/api/bookings` handler must:

1. Start a Prisma transaction
2. For each slot in the request, query the matching `TimeSlot` record (by courtId + date + time)
3. Check that `isBooked` is `false` for every requested slot
4. If any slot is already booked, abort the transaction and return HTTP 409 (Conflict) with a message indicating which slot(s) are taken
5. If all slots are free, set `isBooked: true` on all of them and create the Booking record, all within the same transaction

On the player side, when receiving a 409 response:
- Show an error toast: "One or more slots were just booked by another player"
- Refresh the availability data for that venue
- Keep the bottom sheet open so the player can pick different slots

This prevents two players from booking the same court+time even if they tap "Book" at the same moment.

### Booking Reference ID (orderId)

Auto-generated on booking creation. Format: "CM-" followed by 4 uppercase alphanumeric characters (e.g. "CM-4A7F"). Must be unique. Used for communication between players and venue staff (e.g. when calling the venue about a booking). Displayed prominently on the booking confirmation screen and in My Bookings cards.
...
| Booking Confirmation | Submit booking form | Success state; large orderId (e.g. CM-4A7F); actions |
| My Bookings | Top bar icon on Results | Booking cards; 30s polling; "Updated just now"; filters |
...
- Push notifications for booking status changes (polling used for MVP)
...
5. **Booking submission:** POST to `/api/bookings`. The handler must use a Prisma transaction to atomically check slot availability and create the booking (see "Double Booking Prevention" above). Return booking ID and orderId on success. Return 409 on conflict.

6. **Booking reference (orderId):** Auto-generate on creation using a function like: `"CM-" + randomBytes(2).toString("hex").toUpperCase().slice(0,4)`. Check uniqueness before inserting (retry on collision). Display in booking confirmation, My Bookings cards, and admin views.

7. **Responsive:** The app is mobile-first (max-width 430px centered). It should work on desktop but the primary experience is mobile browser or PWA.

8. **Offline:** Saved venues and user profile should work offline (localStorage). Bookings require network.

9. **My Bookings polling:** Use `setInterval` with 30-second refresh when the My Bookings screen is mounted. Clear the interval on unmount. Listen for `visibilitychange` and only perform a refetch (or let the next tick run) when `document.visibilityState === "visible"` so hidden tabs do not trigger unnecessary API calls.
